...
def plot(a, b, cases, main_label, last_label):
    ...
    plt.rcParams.update({'font.sans-serif': [font_name], 'font.size': 9, 'axes.labelsize': 10, 'xtick.labelsize': 9,
    ...
    ax.set(xlabel='烘干时间 / h', ylabel='干基含水率 / (kg/kg)', xlim=(0, x[-1]), ylim=(0, 2.65))
    ax.legend(loc='upper right')
    save(fig, 'result_q3_代表半径含水率')

    fig, ax = plt.subplots(figsize=(6.3, 3.3), layout='constrained')
    for out, label, color, style in [(a, '尾段均值延拓', COLOR_SEQUENCE[0], '-'),
                                      (b, '末值延拓', COLOR_SEQUENCE[1], '--')]:
    ...
    ax.axhline(0.15, color='#303030', ls=':', label='达标阈值 0.15')
    ax.set(xlim=(left, right), ylim=(0.1497, 0.151), xlabel='烘干时间 / h',
           ylabel='全域最大干基含水率 / (kg/kg)')
    ...
    ax.invert_yaxis()
    ax.set(xlabel='较采用网格与步长的阈值时间差 / s', ylabel='数值设置')
